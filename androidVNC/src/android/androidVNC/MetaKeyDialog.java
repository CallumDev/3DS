...
class MetaKeyDialog extends Dialog implements ConnectionSettable {
									...
									public void onClick(DialogInterface dialog, int which) {
										...
										if (_listId > 1)
										{
											...
											db.execSQL(MessageFormat.format("DELETE FROM {0} WHERE {1} = {2}",
													AbstractMetaKeyBean.GEN_TABLE_NAME, AbstractMetaKeyBean.GEN_FIELD_METALISTID,
													_listId));
											db.execSQL(MessageFormat.format("DELETE FROM {0} WHERE {1} = {2}",
					    	...
					    	Utils.showYesNoPrompt(_canvasActivity, "Delete from list",
					    			...
					    			new OnClickListener() {
										...
										public void onClick(DialogInterface dialog, int which) {
											...
											if (newPos != AdapterView.INVALID_POSITION && newPos < _keysInList.size())
											{
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.itemDeleteKey).setEnabled(_spinnerKeysInList.getSelectedItemPosition()!=AdapterView.INVALID_POSITION);
		return true;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (_lists == null) {
			...
			ImplementationBase.getAll(_database.getReadableDatabase(), MetaList.GEN_TABLE_NAME, _lists, MetaList.GEN_NEW);
		}
	...
	private String getCopyListString()
	{
		if (copyListString==null)
		{
			...
			sb.append(" ( ");
			sb.append(AbstractMetaKeyBean.GEN_FIELD_METALISTID);
			StringBuilder fieldList = new StringBuilder();
			for (Entry<String,Object> s : _currentKeyBean.Gen_getValues().valueSet())
			{
				if (!s.getKey().equals(AbstractMetaKeyBean.GEN_FIELD__ID) && !s.getKey().equals(AbstractMetaKeyBean.GEN_FIELD_METALISTID)) {
					fieldList.append(',');
				...
				if (list.get_Id()==listId)
				{
					...
					Cursor c = _database.getReadableDatabase().rawQuery(
							MessageFormat.format("SELECT * FROM {0} WHERE {1} = {2} ORDER BY KEYDESC",
									AbstractMetaKeyBean.GEN_TABLE_NAME,
									AbstractMetaKeyBean.GEN_FIELD_METALISTID,
									listId),
							EMPTY_ARGS);
					ImplementationBase.Gen_populateFromCursor(
							c,
							_keysInList,
							MetaKeyBean.NEW);
					c.close();
